...
export const actionsCreators = () => {
  return {
    getMenu: (context, menuID) => {
      Vue.http.get('/wp-json/wp-api-menus/v2/menus/' + menuID).then((resp) => {
        let data = resp.body;
    ...
    getWidget(context, name) {
      Vue.http.get('/wp-json/wp-rest-api-sidebars/v1/sidebars/' + name).then(response => {
    ...
    sendContactForm(context, params) {
      ...
      Vue.http.post('/wp-admin/admin-ajax.php', {
        'action': 'send_mail',
        'data': params
      },).then(response => {
